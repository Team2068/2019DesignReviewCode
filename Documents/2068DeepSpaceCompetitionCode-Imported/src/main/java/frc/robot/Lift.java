...
public class Lift {
    ...
    public void baseLiftControl() {
        ...
        if (controller.getY(GenericHID.Hand.kLeft) > 0 ) {
            //System.out.print("go down");
            motor.set(-controller.getY(GenericHID.Hand.kLeft));
        } else if (controller.getY(GenericHID.Hand.kLeft) < 0) {
    ...
    public void steppingLiftControl() {
        ...
        if (controller.getAButtonPressed()) {
            ...
            /*
             * else if(MechanismControl.getHasHatch() && curPosition == 1) { curPosition =
             * curPosition; targetMet = true; }
             */
            else {
                curPosition--;
                targetMet = false;
        ...
        if (curPosition == 0) {
            ...
            if (encoderPosition < selectedHeights[curPosition] && !upperSwitch.get()
                    && !(encoderPosition > selectedHeights[selectedHeights.length - 1] + 10))
